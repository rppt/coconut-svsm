...
use crate::mm::virtualrange::{virt_alloc_range_4k, virt_free_range_4k};
...
const TPM_CRB_REGION_SIZE: usize = PAGE_SIZE;
...
lazy_static! {
    pub static ref TPM_CRB_REGS: SpinLock<&'static mut TpmCrbRegisters> = {
        let crb_pa = PhysAddr::new(TPM_CRB_BASE.try_into().unwrap());
        let flags = PageTable::data_flags();

	let size = PAGE_SIZE;
        let crb_va = match virt_alloc_range_4k(TPM_CRB_REGION_SIZE, PAGE_SIZE) {
	    Ok(v) => v,
	    Err(_r) => panic!("enomem"),
	};

        if let Err(_e) =
	    this_cpu_mut().get_pgtable()
            .map_region_4k(crb_va, crb_va + size, crb_pa, flags) {
		virt_free_range_4k(crb_va, size);
		panic!("enomem");
            }

	if let Err(_e) = this_cpu_mut().get_pgtable().set_shared_4k(crb_va) {
	    let crb_end = crb_va + size;
	    this_cpu_mut().get_pgtable().unmap_region_4k(crb_va, crb_end);
            virt_free_range_4k(crb_va, size);
	    panic!("enomem");
        }
...
impl fmt::Debug for TpmCrbRegisters {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        fmt::Display::fmt("TpmCrbRegisters", f)
    }
}
