...
pub fn tpm2_get_ek_pub() -> Vec<u8> {
    ...
    if size as usize > core::mem::size_of::<TPMT_PUBLIC>() {
        println!("ERROR: TPM2B_PUBLIC.size={:#x} is too big", size);
        return Vec::new();
    }

    // TPM2B_PUBLIC structure
    let out_public: &[u8] = &out_parms[..{size as usize + U16_SIZE}];
    //println!("out_public {:x} {:02x?}", {out_public.len()}, out_public);
    out_public.to_vec()
}

pub fn tpm2_write_report_nvram(report: &[u8]) {
    if report.len() > usize::from(u16::MAX) {
        println!("ERROR: Attestation report size = {:#x} too big", {report.len()});
        return;
    }
    ...
    println!("INFO: Report ({} bytes) written to the TPM NV index {:#x}", {report.len()}, nvindex);
}
